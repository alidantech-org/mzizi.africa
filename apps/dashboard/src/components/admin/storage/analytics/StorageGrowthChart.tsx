...
export default function StorageGrowthChart({ data, loading = false, growthType = 'monthly' }: StorageGrowthChartProps) {
  ...
  const chartData = timelineData.map((timelineItem) => {
    ...
    if (match) {
      return {
        ...
        storage: match.total_size / (1024 * 1024), // Convert to MB
      };
    }
    return timelineItem;
  });
  ...
  const getGrowthTypeLabel = (type: string) => {
    switch (type) {
      case 'daily':
        return 'Daily growth trends';
      case 'weekly':
        return 'Weekly growth trends';
      default:
        return 'Monthly growth trends';
    }
  };
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>{getGrowthTypeLabel(growthType)}</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <CardHeader>
        <div className="flex justify-between flex-col gap-4 md:flex-row items-center">
          <div>
            ...
            {/* <CardDescription className="text-muted">File count and storage size over time</CardDescription> */}
          </div>
        ...
        <div className="h-[300px]">
          <ResponsiveContainer className="p-0 -ml-10 min-w-[110%]" width="" height="100%">
            <ComposedChart data={formattedData}>
              <defs>
                <linearGradient id="fileGrowthGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="rgb(59, 130, 246)" stopOpacity={0.6} />
                  <stop offset="100%" stopColor="rgb(59, 130, 246)" stopOpacity={0.05} />
                </linearGradient>
                <linearGradient id="storageGrowthGradient" x1="0" y1="0" x2="0" y2="1">
              ...
              <XAxis
                ...
                tick={{ fill: 'rgb(107, 114, 128)', fontSize: currentView === 'monthly' ? 9 : 10 }}
                  ...
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="bg-background p-2 border border-gray-300 rounded shadow-sm">
                        ...
                        <p className="text-primary">Files: {data.files.toLocaleString()}</p>
                        <p className="text-accent">Storage: {data.storage.toFixed(2)} MB</p>
                      </div>
                    );
                  }
                  return null;
              ...
              {/* Area fills for background */}
              <Area type="monotone" dataKey="files" stroke="transparent" fill="url(#fileGrowthGradient)" />

              <Area type="monotone" dataKey="storage" stroke="transparent" fill="url(#storageGrowthGradient)" />

              {/* Line charts for trends */}
              <Line type="monotone" dataKey="files" stroke="rgb(59, 130, 246)" strokeWidth={3} dot={false} />

              <Line type="monotone" dataKey="storage" stroke="rgb(34, 197, 94)" strokeWidth={3} dot={false} />

              {/* Scatter points for data points */}
              <Scatter dataKey="files" fill="rgb(59, 130, 246)" shape="circle" />

              <Scatter dataKey="storage" fill="rgb(34, 197, 94)" shape="circle" />
            </ComposedChart>
          </ResponsiveContainer>
        </div>
      </CardContent>
